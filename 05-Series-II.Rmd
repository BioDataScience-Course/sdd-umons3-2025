...
## Décomposition de séries {#decompseries}
...
`r h5p(35, "C05Ha_stationnarisation", height = 270, toc = "Stationnarisation d'une série")`

### Fonction générale de décomposition {#fctgeneraledecomp}
...
Une fois qu'une décomposition est réalisée, il est possible d'extraire les différentes composantes (grâce à `tseries()`) ou une partie d'entre elles (en utilisant la méthode `extract()`) et de les transformer en objet de séries régulières (objet **ts** ou **mts**). On peut ainsi appliquer de nouvelles méthodes de décomposition sur un ou plusieurs composants et approfondir l'analyse des séries par des traitements de décomposition en cascade jusqu'à ce qu'on ait extrait et analysé toute l'information pertinente contenue dans ces séries.

##### À vous de jouer ! {.unnumbered}

`r h5p(34, "C05Hb_tsd", height = 270, toc = "La fonction tsd()")`
...
Outre son graphique qui est des plus utiles pour juger le résultat de notre décomposition visuellement, nous pouvons transformer toutes les composantes obtenues en un objet **mts** à l'aide de `tseries()`, ou alors, extraire une ou plusieurs composantes avec `extract()`. Effectuons les deux successivement.
...
## Filtrage d'une série {#filtrageserie}
...
`r h5p(33, "C05Hc_but_decaverage", height = 270, toc = "Méthode des moyennes mobiles")`
...
### Médianes mobiles {#medianemobile}
...
`r h5p(36, "C05Hd_decmedian", height = 270, toc = "Méthode des médianes mobiles")`
...
### Filtrage par différences {#filtragedifference}
...
`r h5p(32, "C05He_but_decdiff", height = 270, toc = "Méthode des différences")`
...
## Régressions et séries {#regressionseries}
...
### Estimation de la tendance par régression {#estimationregression}
...
L'ANOVA, et les tests de Student sur les paramètres ne peuvent être utilisés ici, même si nous pouvons les calculer à partir de `summary()` ou `anova()`. Pour utiliser ce modèle linéaire pour décomposer la série, nous utilisons `predict()` pour récupérer les valeurs prédites par le modèle pour la tendance, et nous les utilisons dans `decreg()` ou `tsd(methode = "reg")` :
...
$$X_t = A \cdot \cos(2 \pi \omega t + \phi) = \beta_1 \cdot \cos(2 \pi \omega t) + \beta_2 \cdot \sin(2 \pi \omega t)$$
...
$$X_t = \beta_1 \cdot T_1 + \beta_2 \cdot T_2$$

Cette dernière forme est assimilable à une régression multiple sur deux variables $T_1$ et $T_2$ dont l'ordonnée à l'origine vaut zéro, et que nous pouvons ajuster facilement à l'aide de la fonction `lm()`[^05-series-ii-2]. En pratique, nous ne forcerons pas l'ordonnée à l'origine dans notre régression (co-)sinusoïdale, car le signal peut varier autour d'une moyenne qui n'est pas forcément nulle.
...
## Décomposition par LOESS {#decomploess}
...
if (exists("assignment"))
  assignment("C05Ia_tsd", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-025'         = !"{assign_url$C05Ia_tsd}"),
    course.starts = c(
      'S-BIOG-025'         = !"{class1_start(mod, 'C05')}"),
    course.ends = c(
      'S-BIOG-025'         = !"{n3_end(mod, 'C05')}"),
    term = "Q1", level = 3,
...
if (exists("assignment2"))
  assignment2("C04Ga_tseries", part = "II",
    ...
    term = "Q1", level = 4, n = 4,
    toc = "Analyse libre de séries spatio-temporelles (partie II)")
```
...
```{block, type='warning'}
Toutes ces méthodes sont à utiliser **avec précaution**. Il est très facile d’obtenir n’importe quoi si l’on choisit des mauvais paramètres ! Il n’y a pas de règle absolue dans leur choix, étant donné qu’ils sont liés à la nature-même des données à régulariser. Par exemple, il faut être extrêmement attentif à ne pas régulariser à tort des données physico-chimiques (par exemple à l’aide de la méthode des aires avec une très grande fenêtre).
```
...
Dans le cadre de ce cours, la régularisation de série irrégulière est une matière optionnelle. La présentation très succincte ci-dessus vous permet de prendre connaissance de l'existence de ces techniques (faites toujours `?<nom_de_function` pour lire la page d'aide de chaque fonction citée comme point de départ). Revenez vers cette section, et vers les liens qu'elle renferme, si vous vous trouvez confronté à une situation où une régularisation de série est nécessaire. Le [manuel de {pastecs}](https://github.com/phgrosjean/pastecs/blob/master/manual/pastecs_manuel_utilisateur.pdf) renferme un chapitre complet qui décrit en détail comment procéder.
...
## Récapitulatif des exercices {#recapitulatifc05}
