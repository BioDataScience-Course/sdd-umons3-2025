...
Vous devez être à l'aise dans l'utilisation de R, RStudio, (R) Markdown/Quarto, ainsi que Git et GitHub. Si ce n'est pas le cas, l'`r course_link("annexe B du cours de Science des données I", course = 1, "prise")` devraient vous aider à vous remettre à niveau.

##### À vous de jouer ! {.unnumbered}

`r h5p(159, "C01Aa_tools", height = 270, toc = NULL)`
...
`r h5p(160, "C01Ab_git_summary", height = 270, toc = NULL)`
...
Outre les outils, ce cours s'appuie sur des notions que nous avons déjà abordées dans le cours de `r course_link("science des données biologiques 2", course = 2, "cah-kmeans-div")` la notion de **classification non supervisée**. De même, la modélisation (via les **régressions**) a aussi fait l'objet d'une attention particulière dans le `r course_link("cours précédent", course = 2, "lm")`. Toutes ces connaissances vont servir de pilier de base pour ce que nous étudierons ensemble dans ce module, ainsi que dans les deux suivants concernant la classification supervisée.

##### À vous de jouer ! {.unnumbered}

`r h5p(161, "C01Ac_classification", height = 270, toc = NULL)`

Assurez-vous que vous possédez encore bien les bases de l'utilisation de R pour importer, remanier, visualiser des données, pour réaliser un test d'hypothèse, une régression linéaire et une analyse multivariée. Prenez également connaissance des **nouveautés** introduites dans la `r !{"SciViews Box {svbox}"}` au travers du learnr suivant.
...
if (exists("assignment"))
  assignment("C01Ia_debug", part = "I",
    ...
    course.urls = c(
      'S-BIOG-025'         = !"{assign_url$C01Ia_debug}"),
    course.starts = c(
      'S-BIOG-025'         = !"{class1_start(mod, 'C01')}"),
    course.ends = c(
      'S-BIOG-025'         = !"{n3_end(mod, 'C01')}"),
    term = "Q1", level = 3,
    toc = "Débogage de code et ADL (partie I)")
```

`r learnr("C00La_refresh", title = "Rappels et nouveautés de la SciViews Box", toc = NULL)`

## Principes de base {#base}
...
`r h5p(162, "C01Ha_vocabulaire", height = 270, toc = "Vocabulaire de la classification supervisée")`
...
`r h5p(163, "C01Hb_set_apprentissage", height = 270, toc = "Set d'apprentissage et de test")`

### Conditions d'application {#condiapplication}
...
`r h5p(164, "C01Hc_condition_application", height = 270, toc = "Condition d'application")`

## Mesure de performances {#perf}
...
`r h5p(165, "C01Hd_confusion", height = 270, toc = "Vrais et faux positifs ou négatifs")`

### Métriques de performance {#metriqueperformance}
...
`r h5p(168, "C01He_recall", height = 270, toc = "Métriques : rappel")`
...
`r h5p(169, "C01Hf_precision", height = 270, toc = "Choix de métriques")`
...
### Micro- et macro-moyenne des métriques {#micromacrometriques}
...
-   Vous pouvez retrouver des indices supplémentaires via le lien suivant : `r !{"[Statistics calculated on confusion matrix](https://github.com/BioDataScience-Course/sdd_lessons/blob/{acad_year}/C01/more/confusion_matrix.pdf)"}`.
...
## Analyse discriminante linéaire {#analysediscriminante}
...
`r h5p(170, "C01Hg_predict", height = 270, toc = "Déploiement d'un classifieur")`
...
if (exists("assignment"))
  assignment("C01Ib_lda", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-025'         = !"{assign_url$C01Ib_lda}"),
    course.starts = c(
      'S-BIOG-025'         = !"{class1_start(mod, 'C01')}"),
    course.ends = c(
      'S-BIOG-025'         = !"{n3_end(mod, 'C01')}"),
...
## Récapitulatif des exercices {#recapitulatifc01}
