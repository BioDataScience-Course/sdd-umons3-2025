...
-   Appréhender le seuil de détection, ainsi que les courbes ROC et l'AUC comme mesures indépendantes du seuil de détection dans un cas à deux classes
...
2.  Dans notre second cas, **les proportions sont inversées**. Nous avons 20% de A et 80% de B. Même raisonnement : les vrais positifs pour 100 individus seront de 20 (tous les A) et les faux positifs seront 10% de 80, soit 8. La précision devient donc $20 / (20 + 8)$, soit un tout petit peu plus de 71%[^03-classification-iii-1].

[^03-classification-iii-1]: Faites le même calcul du rappel, à la fois pour A et pour B dans les deux cas pour vous convaincre que cette métrique *n'est pas* sensible aux proportions relatives des classes, contrairement à la précision.

##### À vous de jouer ! {.unnumbered}

`r h5p(240, "C03Hb_classifiers", height = 270, toc = "Calcul de précision")`
...
### Probabilités *a priori* {#probaapriori}
...
`r h5p(243, "C03He_prior", height = 270, toc = "Probabilités a priori et set d'apprentissage")`
...
if (exists("assignment"))
  assignment("C03Ia_cardiovascular", part = "I",
    ...
    course.urls = c(
      'S-BIOG-025'         = !"{assign_url$C03Ia_cardiovascular}"),
    course.starts = c(
      'S-BIOG-025'         = !"{class1_start(mod, 'C03')}"),
    course.ends = c(
      'S-BIOG-025'         = !"{n3_end(mod, 'C03')}"),
    term = "Q1", level = 3,
...
Avant d'aborder les courbes ROC, nous devons nous concentrer sur le **seuil de détection** (*cutoff* en anglais). Dans un classifieur à deux classes avec la forêt aléatoire, un vote à la majorité des arbres revient à dire que plus de la moitié des arbres a voté pour la classe retenue. Ceci correspond donc à un seuil de 50%. Mais qu'est-ce qui vous empêcherait d'utiliser un seuil différent ? Intuitivement, nous réalisons tout de suite que si nous **durcissons** le seuil pour la classe cible (par exemple, un individu ne sera déclaré de cette classe, appelée *A*, qu'à partir du moment où 70% ou plus des arbres ont voté en ce sens), nous aurons au final *moins* d'individus *A* dans notre prédiction. Cela aura probablement pour conséquence d'augmenter la précision. En effet, plus de cas difficiles à discriminer auront été attribués à la classe *B*, or il est possible de montrer qu'une partie importante de l'erreur se situe justement dans ces cas moins nets. En revanche, cela diminue le rappel, car parmi ces cas difficiles, il y a tout de même des *A* qui ne sont plus classés comme tels. C'est le fameux compromis rappel - précision. Choisir un seuil moins sensible, par exemple 30%, aura l'effet inverse d'augmenter les prédictions *A*, ainsi que le rappel, mais de diminuer précision et sensibilité. Sachant cela, nous pouvons nous demander s'il existe un seuil optimal. La courbe ROC permet de le déterminer.
...
Nous utilisons le package {ROCR} (déjà chargé via `SciViews::R("ml")`) qui demande uniquement les appartenances à la classe cible (ici `pos`, donc la seconde colonne), ainsi qu'un vecteur indiquant par 0 ou 1 si l'individu appartient effectivement ou non à cette classe cible. À l'aide de ces données, nous construisons un objet **prediction** à l'aide de la fonction du même nom. À partir de là, nous calculons ensuite un objet **performance** en indiquant les deux métriques que nous voulons utiliser ("true positive rate" `"tpr"` et "false positive rate" `"fpr"` pour la courbe ROC), et nous réalisons ensuite le graphique[^03-classification-iii-3].

[^03-classification-iii-3]: Cela peut paraître compliqué. En réalité, cette approche permet de réaliser plusieurs autres graphiques intéressants que nous ne détaillerons pas ici. Nous vous renvoyons à la documentation du package {ROCR} si vous êtes intéressé.
...
1.  La frontière entre les deux classes n'est pas forcément linéaire, et en pratique, la plupart du temps elle ne l'est pas.
...
La transformation appliquée est une fonction mathématique appelée **noyau** (*kernel* en anglais). Un certain nombre de noyaux ont été définis pour transformer des frontières de forme précise. Par exemple, si la frontière initiale est circulaire, un noyau existe pour linéariser cette frontière en transformant notre plan en hyperbole tridimensionnelle par le calcul d'une troisième dimension telle que *x~3~ = x~1~^2^ + x~2~^2^*. Découper notre hyperbole 3D par une frontière plane revient alors à découper notre plan initial à l'aide d'une frontière circulaire. Ceci est illustré dans la vidéo suivante.
...
`r h5p(245, "C03Hg_svm", height = 270, toc = "SVM et nombre de classes")`
...
On se rapproche des scores de la forêt aléatoire ici avec un *F* micro et macro de 74%. Le noyau sigmoïde est ici désastreux (non illustré), mais le noyau polynomial de degré 2 et avec un `coef0 = 1` donne ceci :
...
## Réseaux de neurones artificiels {#reseauxneurones}
...
![Représentation simplifiée d'une partie d'un réseau de neurones à une seule couche cachée (en bleu) et complètement connectée. En vert, la couche sensorielle qui reçoit les valeurs des attributs. En jaune, le neurone qui prédit une classe en jaune (source Wikipedia).](images/03-classification3/reseau-neurones.png)

### Pima avec réseau de neurones {#pimareseauneurones}
...
if (exists("assignment2"))
  assignment2("C01Ga_metrics", part = "III",
    ...
    course.urls = c(
      'S-BIOG-025'         = !"{assign_url$C01Ga_metrics}"),
    course.starts = c(
      'S-BIOG-025'         = !"{class2_start(mod, 'C01')}"),
    course.ends = c(
      'S-BIOG-025'         = !"{n4_end(mod, 'C03')}"),
    term = "Q1", level = 0, n = 4,
...
if (exists("challenge"))
  challenge("C03Ca_ml_wine", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-025' = !"{assign_url$C03Ca_ml_wine}",
      'sdd3late'   = !"{assign_url$C03Ca_ml_wine}"),
    course.starts = c(
      'S-BIOG-025' = !"{mod['C03', 'challenge']} 09:45:00",
      'sdd3late'   = !"{mod['C03', 'challenge']} 09:45:00"),
    course.ends = c(
      'S-BIOG-025' = !"{mod['C03', 'challenge']} 11:30:00",
      'sdd3late'   = !"{mod['C03', 'challenge']} 11:30:00"),
    level = 4, n = 1,
...
## Récapitulatif des exercices {#recapitulatifC03}
