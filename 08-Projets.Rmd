...
Dans ce module, nous traiterons de RStudio et des projets RStudio, de R Markdown, de git et GitHub, ainsi que de divers outils de R pour importer, remanier, décrire et analyser des données biologiques. Toutes ces notions doivent être parfaitement connues avant d'aborder le présent module qui va vous emmener un niveau plus loin vers un gros projet constitué de plusieurs étapes successives dans l'analyse. Toutes ces notions ont été utilisées en routine dans les trois cours précédents, `r !{"[SDD I](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-{YYYY}/)"}`, `r !{"[SDD II](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons2-{YYYY}/)"}` et SDD III (les cinq premiers modules du présent cours). Ainsi, si vous avez bien suivi ces trois cours-là, vous êtes en principe parfaitement préparé pour aborder cette matière.

## Structurer un projet {#structurer}
...
### Qualités d'un bon projet {#qualiteprojet}
...
### Structure conseillée {#structureconseillee}
...
-   `r course_link("Acquisition de données", course = 1, "acquisition")`

-   `r course_link("Encodage des données dans un tableau cas par variables", course = 1, "tableaudonnees", anchor = "tableaux-cas-par-variables" )`

-   `r course_link("Ajout de métadonnées", course = 1, "tableaudonnees", anchor = "m%C3%A9tadonn%C3%A9es" )` avec utilisation d'un `r course_link("dictionnaire des données", course = 1, "tableaudonnees", anchor = "dictionnaire-des-donn%C3%A9es" )`

Il est de plus intéressant de pouvoir rendre les données de sa recherche accessibles en respectant les normes et les règles de l'`r course_link("Open Data", course = 2, "donneesouvertes")`.
...
### Données brutes et retravaillées {#donneesbrutesretravaillees}
...
### Format des données {#formatdonnees}
...
## Documents de synthèse {#docsynthese}
...
```{r assign_D08Ga_project_I, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("D08Ga_project", part = "I",
    ...
    term = "Q1", level = 4, n = 1,
    toc = "Organisation d'un projet (partie I)")
```
...
### Exclusion de fichiers du dépôt git {#exclusiongit}
...
## Récapitulatif des exercices {#recapitulatifd08}
